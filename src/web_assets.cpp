...
// web_server_manager.cpp's 1955-LOC pile. These are the embedded copies
// served when LittleFS doesn't have the matching file at /data/. Per
// architecture.md §"UI serving model", the fallback UI must stay
// behaviorally aligned with the LittleFS assets.
//
// Co-located in one translation unit so when the asset markup changes
// the diff is in one place, and web_server_manager.cpp stops carrying
// ~1230 LOC of asset noise. `static` was dropped to give the blobs
// external linkage; web_assets.h declares them `extern`. PROGMEM
// section attribute preserved — these still live in flash, not RAM.

#include <Arduino.h>
#include "web_assets.h"

const char FALLBACK_INDEX_HTML[] PROGMEM = R"HTML(
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
const char FALLBACK_STYLE_CSS[] PROGMEM = R"CSS(
* {
...
const char FALLBACK_APP_JS[] PROGMEM = R"JS(
const state = {
  status: null,
  config: null,
