...
// Heap-pressure / proactive-restart / compact-heartbeat constants moved
// to central_client_heap.cpp in the 0.2.40 refactor — every reference to
// those constants now lives in that file with the methods that use them.
...
// shouldUseCompactHeartbeat() moved to central_client_heap.cpp in
// the 0.2.40 refactor.

// maybeHeapPressureRestart(), heapTrajectoryRecovering(), sampleHeap(),
// and serializeHeapTrajectory() moved to central_client_heap.cpp in
// the 0.2.40 refactor. Same class — C++ allows splitting a class's
// method bodies across multiple translation units, and PlatformIO
// compiles every .cpp under src/.
